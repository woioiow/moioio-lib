...
public class Element
{
    ...
    public void setChildren(List<Element> v)
    {
        children = v;
    }
    ...
    public static Element[] getElements(List<Element> v)
    {
        if(v==null)
        {
            return null;
        ...
        Element[] el = v.toArray(new Element[size]);
//        v.copyInto(el);
        return el;
    }
...
//                    MyLog.debug("src:"+root.getAttr("src"));
    ...
    public static String getTag(String str)
    {
        ...
        String tag = str.substring(tmp, end);
...
//        MyLog.debug(s+getTag()+":"+ss);
        for (int i = 0; i < ss; i++)
        {
            Element em = this.children.get(i);
//            MyLog.debug("tag:"+em.tag);
    ...
    {
        return toXmlString(this,isFormat);
    }



    private String toXmlString(Element node,boolean isFormat)
    {
        ...
        if(node.isRoot)
        {
            ...
            {
                stringBuffer.append(getXml(element,0,isFormat));
            }
        }
        else
        {
            stringBuffer.append(getXml(node,0,isFormat));
        }
    ...
    public String getXml(Element node,int space,boolean isFormat)
    {
        StringBuffer sb = new StringBuffer();
        sb.append(makeSpace(space,isFormat));
        sb.append("<");
        sb.append(node.getTag());
        sb.append(" ");

        List<String> enu = node.getAttrs();
        for(String key:enu)
        {
            sb.append(key);
            sb.append("=\"");
            sb.append(node.getAttr(key));
            sb.append("\"");
            sb.append(" ");
        }
        int size = node.size();
        if(size==0)
        {
            if(StringUtil.isNull(node.getText()))
            {
        ...
        else
        {
            ...
            {
                Element em = node.getChildren().get(i);
                sb.append(getXml(em,space+1,isFormat));
            }
            sb.append(makeSpace(space,isFormat));
            sb.append("</");
            sb.append(node.getTag());
    ...
    private String makeSpace(int num,boolean isF)
    {
        StringBuffer stringBuffer = new StringBuffer();
        if(isF)
        {
